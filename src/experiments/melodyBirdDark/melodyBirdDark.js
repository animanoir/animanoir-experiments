console.log("melodyBirdDark.js running...")
...
const samplePlayer = new Tone.Player({
  url: "/musicsfx/melody-bird-dark.ogg",
  loop: true
}).toDestination();

const stormPlayer = new Tone.Player({
  url: "/musicsfx/distant-storm.ogg",
  loop: true,
  volume: -12 // Start with a lower volume for the storm sound
}).toDestination();
...
// Update the GUI onChange handler to control both players
gui.add(guiParameters , 'isMusicOn').name('Music:').onChange((value) => {
  isMusicOn = value;
  if (isMusicOn) {
    // Only start if Tone.js is ready
    Tone.loaded().then(() => {
      if (!samplePlayer.state || samplePlayer.state === 'stopped') {
        samplePlayer.start();
      }
      if (!stormPlayer.state || stormPlayer.state === 'stopped') {
        stormPlayer.start();
      }
    });
  } else {
